...
def pol_selfcal(
    ...
    """
    A single polarisation self-calibration round
    ...
    try:
        ...
        # Determine temporal chunking
        ###############################
        nintervals = max(1,round(total_time/240.0))
        ...
        # Final frequency averaged images for backup or calculating dynamic ranges
        #######################################################################
        final_image = prefix.replace("present", f"{round_number}") + "_IQUV_image.fits"
        final_model = prefix.replace("present", f"{round_number}") + "_IQUV_model.fits"
        final_residual = (
            prefix.replace("present", f"{round_number}") + "_IQUV_residual.fits"
        )
